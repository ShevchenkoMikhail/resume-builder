    ...
    useEffect(() => {
        const fetchResume = async () => {
            try {
                ...
                // Убедимся, что в данных есть все необходимые поля
                if (!data.templateSettings) {
                    ...
                    if (!data.templateSettings.fontFamily) {
                        data.templateSettings.fontFamily = 'Arial, sans-serif';
                    }
                    if (!data.templateSettings.lineSpacing) {
                        data.templateSettings.lineSpacing = 1.5;
                    }
                }
                ...
                setError(error.message || 'Произошла ошибка при загрузке резюме');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || 'Не удалось обновить резюме');
            }

            router.push(`/dashboard`);
        } catch (error) {
            console.error('Ошибка при обновлении резюме:', error);
            setError(error.message || 'Произошла ошибка при обновлении резюме');
        } finally {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="mt-2">Загрузка резюме...</p>
                </div>
            </div>
        );
    }
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="flex justify-between items-center mb-6">
                            <h1 className="text-3xl font-bold text-gray-900">
                                {t('resume.edit', 'Редактирование резюме')}
                            </h1>
                        </div>
                            ...
                            <div className="bg-white shadow overflow-hidden sm:rounded-lg overflow-y-auto" style={{ maxHeight: 'calc(100vh - 200px)' }}>
                                <form onSubmit={handleSubmit}>
                                    ...
                                    <div className="px-4 py-5 sm:px-6 flex justify-end space-x-3 border-t border-gray-200">
                                        ...
                                        >
                                            {saveLoading ? t('button.saving', 'Сохранение...') : t('button.save', 'Сохранить изменения')}
                                        </Button>
                                    </div>
                                </form>
                            </div>
